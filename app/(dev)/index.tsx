...
const SCREENS: Screen[] = [
  ...
  { group: '온보딩', label: '가계부 만들기', route: '/(onboarding)/create-couple' },
  { group: '온보딩', label: '초대코드 합류', route: '/(onboarding)/join-couple' },
...
export default function DevPreviewScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-cream" contentContainerClassName="px-6 pt-16 pb-12">
      <Text className="font-ibm-bold text-[28px] text-brown mb-1">Screen Preview</Text>
          ...
          <View className="gap-2">
            {SCREENS.filter((s) => s.group === group).map((screen) => (
              <TouchableOpacity
                ...
                style={rowShadow}
              >
                <Text className="font-ibm-semibold text-base text-brown">{screen.label}</Text>
                <Text className="font-ibm-regular text-xs text-brown/40">{screen.route}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
