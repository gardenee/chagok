...
export default function FixedScreen() {
  ...
  function openEdit(item: FixedExpense) {
    setModal({
      visible: true,
      editingId: item.id,
      form: { name: item.name, amount: String(item.amount), due_day: item.due_day },
    });
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
  }
    ...
    <SafeAreaView className="flex-1 bg-cream">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View className="flex-row items-center justify-between px-6 pt-6 pb-2">
          <Text className="font-ibm-bold text-2xl text-brown">고정지출</Text>
          <TouchableOpacity
            onPress={openCreate}
            className="w-10 h-10 rounded-full bg-butter items-center justify-center"
            activeOpacity={0.7}
            style={{ shadowColor: Colors.butter, shadowOpacity: 0.8, shadowRadius: 10, shadowOffset: { width: 0, height: 3 } }}
          >
        ...
        <View
          ...
          style={{ shadowColor: Colors.butter, shadowOpacity: 0.7, shadowRadius: 16, shadowOffset: { width: 0, height: 4 } }}
        >
          <View className="flex-row items-center gap-2 mb-1">
            <Wallet size={16} color={Colors.brown} strokeWidth={2.5} />
            <Text className="font-ibm-semibold text-sm text-brown/70">매월 고정지출</Text>
          </View>
          <Text className="font-ibm-bold text-3xl text-brown">
            {formatAmount(totalAmount)}원
          </Text>
          <Text className="font-ibm-regular text-xs text-brown/50 mt-1">
        ...
        <View className="mx-4 mt-5">
          {isLoading ? (
            ...
            <View className="bg-cream-dark/40 rounded-3xl py-12 items-center gap-3">
              <Repeat size={28} color={Colors.brown + '30'} strokeWidth={2} />
              <Text className="font-ibm-semibold text-sm text-brown/40">등록된 고정지출이 없어요</Text>
              <Text className="font-ibm-regular text-xs text-brown/30">월세, 구독, 보험 등을 등록해보세요</Text>
            </View>
          ) : (
            <View className="gap-2.5">
              {fixedExpenses.map(item => (
                <TouchableOpacity key={item.id} onPress={() => openEdit(item)} activeOpacity={0.8}>
                  <View
                    className="bg-white rounded-3xl px-4 py-4 flex-row items-center gap-3"
                    style={{ shadowColor: Colors.brown, shadowOpacity: 0.07, shadowRadius: 10, shadowOffset: { width: 0, height: 2 }, elevation: 2 }}
                  >
                    {/* 아이콘 */}
                    ...
                    <View className="flex-1">
                      <Text className="font-ibm-semibold text-sm text-brown">{item.name}</Text>
                      <Text className="font-ibm-regular text-xs text-brown/50 mt-0.5">
      ...
      >
        ...
        >
          ...
          <View
            ...
            style={{ shadowColor: Colors.brown, shadowOpacity: 0.15, shadowRadius: 20, shadowOffset: { width: 0, height: -4 } }}
          >
            {/* 모달 헤더 */}
            ...
            <View
              ...
              style={{ shadowColor: Colors.brown, shadowOpacity: 0.06, shadowRadius: 8, shadowOffset: { width: 0, height: 2 } }}
            >
              <TextInput
            ...
            {/* 금액 */}
            <View
              className="bg-white rounded-2xl px-4 py-3.5 mb-4 flex-row items-center"
              style={{ shadowColor: Colors.brown, shadowOpacity: 0.06, shadowRadius: 8, shadowOffset: { width: 0, height: 2 } }}
            >
              ...
              <TextInput
                ...
                onChangeText={v =>
                  setModal(s => ({ ...s, form: { ...s.form, amount: v.replace(/[^0-9]/g, '') } }))
                }
              />
            </View>
            ...
            {/* 납부일 */}
            <View className="mb-6">
              <Text className="font-ibm-semibold text-xs text-brown/60 mb-2 ml-1">납부일</Text>
              <View
              ...
              >
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <ChevronLeft size={18} color={modal.form.due_day <= 1 ? Colors.brown + '30' : Colors.brown} strokeWidth={2.5} />
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <ChevronRight size={18} color={modal.form.due_day >= 31 ? Colors.brown + '30' : Colors.brown} strokeWidth={2.5} />
                </TouchableOpacity>
              </View>
            </View>
