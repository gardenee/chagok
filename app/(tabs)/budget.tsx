...
const COLOR_OPTIONS = [
  '#F7B8A0', '#D4C5F0', '#FAD97A', '#A8D8B0',
  '#F0C5D5', '#B5D5F0', '#F5D0A0', '#C5E8D5',
  '#E0B5D5', '#B5C8E8', '#E8D8B0', '#D0E8B5',
];
...
const INITIAL_FORM: FormData = { name: '', icon: 'shopping', color: COLOR_OPTIONS[0], budget_amount: '' };
...
export default function BudgetTab() {
  ...
  const totalSpent = categories.reduce((s, c) => s + (spendingByCategory[c.id] ?? 0), 0);
  const totalRemaining = totalBudget - totalSpent;
  const totalRatio = totalBudget > 0 ? Math.min(totalSpent / totalBudget, 1) : 0;
  ...
  function openEdit(c: Category) {
    setModal({ visible: true, editingId: c.id, form: { name: c.name, icon: c.icon, color: c.color, budget_amount: String(c.budget_amount) } });
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
  }

  async function handleSave() {
    const name = modal.form.name.trim();
    const amount = parseInt(modal.form.budget_amount.replace(/[^0-9]/g, ''), 10);
    if (!name) { Alert.alert('입력 오류', '카테고리 이름을 입력해주세요'); return; }
    if (!amount || amount <= 0) { Alert.alert('입력 오류', '예산을 올바르게 입력해주세요'); return; }
    try {
      if (modal.editingId) {
        await updateCategory.mutateAsync({ id: modal.editingId, name, icon: modal.form.icon, color: modal.form.color, budget_amount: amount });
      } else {
        await createCategory.mutateAsync({ name, icon: modal.form.icon, color: modal.form.color, budget_amount: amount, sort_order: categories.length });
      }
    ...
    <SafeAreaView className="flex-1 bg-cream">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <View className="flex-row items-center justify-between px-6 pt-6 pb-2">
          <Text className="font-ibm-bold text-2xl text-brown">예산·결산</Text>
          <TouchableOpacity
            onPress={openCreate}
        ...
        <View className="flex-row items-center justify-center gap-5 px-6 pt-1 pb-3">
          ...
          <Text className="font-ibm-semibold text-base text-brown w-24 text-center">{year}년 {month + 1}월</Text>
        ...
        {totalBudget > 0 && (
          <View
            ...
            style={{ backgroundColor: isOver ? Colors.peach : Colors.butter, shadowColor: Colors.butter, shadowOpacity: 0.7, shadowRadius: 16, shadowOffset: { width: 0, height: 4 } }}
          >
            <View className="flex-row justify-between items-start mb-3">
              <View>
                <Text className="font-ibm-regular text-xs text-brown/60 mb-0.5">이번 달 지출</Text>
                <Text className="font-ibm-bold text-2xl text-brown">{formatAmount(totalSpent)}원</Text>
              </View>
              <View className="items-end">
                <Text className="font-ibm-regular text-xs text-brown/60 mb-0.5">총 예산</Text>
                <Text className="font-ibm-semibold text-base text-brown">{formatAmount(totalBudget)}원</Text>
              </View>
            </View>

            {/* 전체 프로그레스 바 */}
            <View className="bg-brown/10 rounded-full h-2 mb-2">
              <View
                className="h-2 rounded-full bg-brown/50"
        ...
        <View className="mx-4 mt-5">
          <Text className="font-ibm-bold text-base text-brown mb-3">카테고리별 현황</Text>

          {isLoading ? (
            <View className="py-12 items-center"><ActivityIndicator color={Colors.butter} /></View>
          ) : categories.length === 0 ? (
            <View className="bg-cream-dark/40 rounded-3xl py-12 items-center gap-3">
              <Wallet size={32} color={Colors.brown + '30'} strokeWidth={1.5} />
              <Text className="font-ibm-semibold text-sm text-brown/40">카테고리가 없어요</Text>
            ...
            <View className="gap-2.5">
              {categories.map(c => {
                ...
                const ratio = c.budget_amount > 0 ? Math.min(spent / c.budget_amount, 1) : 0;
                const over = spent > c.budget_amount && c.budget_amount > 0;
                const remaining = c.budget_amount - spent;
                return (
                  <TouchableOpacity key={c.id} onPress={() => openEdit(c)} activeOpacity={0.8}>
                    <View
                    ...
                    >
                      <View className="flex-row items-center gap-3 mb-3">
                        <View className="w-10 h-10 rounded-2xl items-center justify-center" style={{ backgroundColor: c.color + '55' }}>
                          <CategoryIcon iconKey={c.icon} color={c.color} />
                        </View>
                        <View className="flex-1">
                          <Text className="font-ibm-semibold text-sm text-brown">{c.name}</Text>
                          <Text className="font-ibm-regular text-xs text-brown/50">
                            예산 {formatAmount(c.budget_amount)}원
                          </Text>
                        </View>
                        <View className="items-end">
                          <Text className={`font-ibm-bold text-sm ${over ? 'text-peach' : 'text-brown'}`}>
                            {formatAmount(spent)}원
                          </Text>
                          <Text className="font-ibm-regular text-[10px] text-brown/40">
                            {over ? `${formatAmount(spent - c.budget_amount)}원 초과` : `${formatAmount(remaining)}원 남음`}
                          </Text>
                        </View>
                      </View>
                      ...
                      <View className="bg-cream-dark rounded-full h-1.5">
                        <View
                          ...
                          style={{ width: `${ratio * 100}%`, backgroundColor: over ? Colors.peach : c.color }}
                        />
                      </View>
                    </View>
                  </TouchableOpacity>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 justify-end">
          ...
          <View className="bg-cream rounded-t-3xl px-6 pt-5 pb-10" style={{ shadowColor: Colors.brown, shadowOpacity: 0.15, shadowRadius: 20, shadowOffset: { width: 0, height: -4 } }}>
            <View className="flex-row items-center justify-between mb-5">
              <Text className="font-ibm-bold text-lg text-brown">{modal.editingId ? '카테고리 수정' : '카테고리 추가'}</Text>
            ...
            <Text className="font-ibm-semibold text-xs text-brown/50 mb-2 ml-1">아이콘</Text>
              ...
              <View className="flex-row gap-2 pr-2">
                {Object.entries(ICON_MAP).map(([key, Icon]) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={key}
                      onPress={() => setModal(s => ({ ...s, form: { ...s.form, icon: key } }))}
                      className={`w-12 h-12 rounded-2xl items-center justify-center ${isSelected ? 'bg-butter' : 'bg-white'}`}
                      activeOpacity={0.7}
                      style={{ shadowColor: Colors.brown, shadowOpacity: 0.06, shadowRadius: 6, shadowOffset: { width: 0, height: 1 } }}
                    >
                      <Icon size={20} color={isSelected ? Colors.brown : Colors.brown + '50'} strokeWidth={2.5} />
                    </TouchableOpacity>
                  );
                })}
              </View>
            ...
            <Text className="font-ibm-semibold text-xs text-brown/50 mb-2 ml-1">색상</Text>
            <View className="flex-row flex-wrap gap-2 mb-4">
              {COLOR_OPTIONS.map(color => {
                ...
                return (
                  <TouchableOpacity
                    key={color}
                    onPress={() => setModal(s => ({ ...s, form: { ...s.form, color } }))}
                    className="w-9 h-9 rounded-full items-center justify-center"
                    style={{ backgroundColor: color, borderWidth: isSelected ? 2.5 : 0, borderColor: Colors.brown }}
                    activeOpacity={0.7}
                  >
                    {isSelected && <Check size={14} color={Colors.brown} strokeWidth={3} />}
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View className="bg-white rounded-2xl px-4 py-3.5 mb-5 flex-row items-center" style={{ shadowColor: Colors.brown, shadowOpacity: 0.06, shadowRadius: 8, shadowOffset: { width: 0, height: 2 } }}>
              ...
              <TextInput
                ...
                onChangeText={v => setModal(s => ({ ...s, form: { ...s.form, budget_amount: v.replace(/[^0-9]/g, '') } }))}
              />
